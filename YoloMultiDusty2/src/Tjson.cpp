...
Tjson::Tjson(): UsedStreams(0),
                ...
                MJPEG_Port(0),
                DnnRect(0,0,WORK_WIDTH,WORK_HEIGHT)
{
    Jvalid=false;
}
//----------------------------------------------------------------------------------------
...
bool Tjson::GetSettings(void)
{
    ...
    if(Jvalid){
        ...
        if(!GetSetting(j["DNN_Rect"],"x_offset",DnnRect.x))     std::cout << "Using default value" << std::endl;
        if(!GetSetting(j["DNN_Rect"],"y_offset",DnnRect.y))     std::cout << "Using default value" << std::endl;
        if(!GetSetting(j["DNN_Rect"],"width",DnnRect.width))    std::cout << "Using default value" << std::endl;
        if(!GetSetting(j["DNN_Rect"],"height",DnnRect.height))  std::cout << "Using default value" << std::endl;
