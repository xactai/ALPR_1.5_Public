#include "FPS.h"
#define FPS_AVERAGE 5  //number of frames per Far[] entry.
//---------------------------------------------------------------------------
TFPS::TFPS()
{
    Cnt=0;
...
void TFPS::Update(void)
{
    ...
    if(FirstInit){
        ...
        if(FirstFull){
            ...
            FrameRate=(FPS_AVERAGE*15*1000.0)/f;        //f in milliseconds (15 because we skipped i==Fcnt)
        }
        else{
            for(f=0.0, i=0;i<Fcnt;i++) f+=Far[i];
            FrameRate=(FPS_AVERAGE*Fcnt*1000.0)/f;      //f in milliseconds
        }
    }
}
//---------------------------------------------------------------------------
